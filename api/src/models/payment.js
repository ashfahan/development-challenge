...
class Payment {
  ...
  getAll(pageSize = 10, pageNumber = 1) {
    const paginatedResults = this.paginateResult(this.state, pageSize, pageNumber);
  ...
  create({ paymentAmount, applicationUuid }) {
    if (applicationUuid) {
      const applicationExists = this.state.filter((ap) => ap.applicationUuid === applicationUuid);
      if (!applicationExists) {
        return { success: false, message: `Application not found or in process.` };
      }
    }
    const record = { uuid: uuid(), paymentMethod: "ACH", applicationUuid: applicationUuid, paymentAmount: paymentAmount };
    this.state.push(record);
    return record;
  }

  paginateResult(array, pageSize, pageNumber) {
